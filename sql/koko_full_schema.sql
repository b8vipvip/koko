...
-- Full schema for a fresh Koko database. Compatible with MySQL 5.7 / 8.0.
-- This file creates tables only; it does not insert real users, phones, tokens,
-- redeem codes, emails, passwords, or production secrets.
--
-- For existing production databases, DO NOT run this file. Use
-- migrations/001_normalize_redeem_code.sql after taking a mysqldump backup.
...
CREATE TABLE IF NOT EXISTS `code_data` (
  ...
  `code` varchar(255) NOT NULL COMMENT '券码/链接',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='优惠券队列表';
...
CREATE TABLE IF NOT EXISTS `device_fund_details` (
  ...
  PRIMARY KEY (`id`),
  KEY `idx_device_time_id` (`device_id`,`operation_time`,`id`),
...
CREATE TABLE IF NOT EXISTS `img_data` (
  ...
  KEY `idx_status_created` (`status`,`create_date`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='截图/图片数据';

CREATE TABLE IF NOT EXISTS `order_data` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `orderID` varchar(255) DEFAULT NULL COMMENT 'legacy redeem code',
  `redeem_code` varchar(255) DEFAULT NULL COMMENT 'canonical redeem code; mirrors orderID',
  `status` int(11) DEFAULT NULL COMMENT '状态',
  `type` varchar(255) DEFAULT NULL COMMENT '类型',
  `xp` double DEFAULT NULL COMMENT '经验/价格',
  `result` varchar(255) DEFAULT NULL COMMENT '结果',
  `llp` varchar(255) DEFAULT NULL COMMENT '利润/售价',
  `time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `phone` varchar(255) DEFAULT NULL COMMENT '手机号',
  `email` varchar(255) DEFAULT NULL COMMENT '邮箱',
  `czp` varchar(255) DEFAULT NULL COMMENT '充值成本/支出',
  ...
  KEY `idx_redeem_code_time` (`redeem_code`,`time`),
  KEY `idx_processed_dev` (`processed`,`dev`),
  KEY `idx_status_time` (`status`,`time`),
  KEY `idx_phone_time` (`phone`,`time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='充值历史结果';

CREATE TABLE IF NOT EXISTS `order_data_anj` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `orderID` varchar(255) NOT NULL COMMENT 'legacy redeem code',
  `redeem_code` varchar(255) DEFAULT NULL COMMENT 'canonical redeem code; mirrors orderID',
  ...
  KEY `idx_redeem_code_status` (`redeem_code`,`status`),
  KEY `idx_type_xp_status` (`type`,`xp`,`status`),
  KEY `idx_processed` (`processed`),
  KEY `idx_phone_time` (`phone`,`time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='兑换码/订单同步表';

CREATE TABLE IF NOT EXISTS `order_id` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `orderID` varchar(255) DEFAULT NULL COMMENT 'legacy redeem code',
  `redeem_code` varchar(255) DEFAULT NULL COMMENT 'canonical redeem code; mirrors orderID',
  `status` int(11) DEFAULT NULL COMMENT '1=可用,2=锁定,其他=已用/异常',
  `type` varchar(255) DEFAULT NULL,
  ...
  KEY `idx_orderID_status` (`orderID`,`status`),
  KEY `idx_redeem_code_status` (`redeem_code`,`status`),
  KEY `idx_type_xp_status_get` (`type`,`xp`,`status`,`getstatus`),
  KEY `idx_admin_extract` (`type`,`xp`,`status`,`getstatus`,`getadminkami`,`91kami`,`adminkami`),
  KEY `idx_status_created` (`status`,`created_at`),
  KEY `idx_phone_time` (`phone`,`time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='兑换码库存主表';
...
CREATE TABLE IF NOT EXISTS `run_status` (
  ...
  PRIMARY KEY (`id`),
  KEY `idx_dev_created` (`dev`,`create_date`),
  KEY `idx_status_created` (`status`,`create_date`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='设备心跳状态';

CREATE TABLE IF NOT EXISTS `submissions` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `idempotency_key` varchar(128) NOT NULL,
  `order_id` varchar(64) NOT NULL COMMENT 'legacy request field; same business meaning as redeem_code',
  `redeem_code` varchar(64) DEFAULT NULL COMMENT 'canonical redeem code; mirrors order_id',
  `response_json` text NOT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_idempotency_key` (`idempotency_key`),
  KEY `idx_order_id_created` (`order_id`,`created_at`),
  KEY `idx_redeem_code_created` (`redeem_code`,`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='提交幂等记录';
...
CREATE TABLE IF NOT EXISTS `tel_data` (
  ...
  `tel` varchar(50) DEFAULT NULL COMMENT 'phone',
  `yzm` varchar(255) DEFAULT NULL COMMENT 'verification code',
  `details` text,
  ...
  `orderID` varchar(50) DEFAULT NULL COMMENT 'legacy redeem code',
  `order_id` varchar(50) DEFAULT NULL COMMENT 'legacy lowercase alias for redeem code',
  `redeem_code` varchar(50) DEFAULT NULL COMMENT 'canonical redeem code; mirrors orderID/order_id',
  ...
  KEY `idx_tel_created` (`tel`,`create_date`),
  KEY `idx_orderID_status` (`orderID`,`status`),
  KEY `idx_order_id_status` (`order_id`,`status`),
  KEY `idx_redeem_code_status` (`redeem_code`,`status`),
  KEY `idx_status_yzm_created` (`status`,`yzm_status`,`create_date`),
  KEY `idx_r_c_status` (`r_status`,`c_status`),
  KEY `idx_yzm_status` (`yzm_status`),
  KEY `idx_dev_created` (`dev`,`create_date`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='短信/充值流水';

CREATE TABLE IF NOT EXISTS `user_data` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `phone` varchar(20) NOT NULL,
  `code` varchar(10) NOT NULL,
  `order_id` varchar(50) DEFAULT NULL COMMENT 'legacy request field; same business meaning as redeem_code',
  `redeem_code` varchar(50) DEFAULT NULL COMMENT 'canonical redeem code; mirrors order_id',
  ...
  PRIMARY KEY (`id`),
  KEY `idx_phone_created` (`phone`,`create_date`),
  KEY `idx_order_id_created` (`order_id`,`create_date`),
  KEY `idx_redeem_code_created` (`redeem_code`,`create_date`),
  KEY `idx_status_created` (`status`,`create_date`),
  KEY `idx_submitstatus` (`submitstatus`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户提交充值记录';
