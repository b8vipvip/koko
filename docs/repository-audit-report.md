...
- PHP 与 Python 现在共用同一套 `/opt/koko/.env` 字段：`DB_HOST`、`DB_PORT`、`DB_USER`、`DB_PASSWORD`、`DB_NAME`、`DB_CHARSET`、`SMTP_*`、`MAIL_*`、`ADMIN_API_TOKEN`。
- 新增 `admin/lib/config.php`：优先读取 `getenv()` / `$_ENV`，然后尝试解析 `PHP_ENV_FILE`、`/opt/koko/.env`、项目根目录 `.env`、`admin/.env`；同时提供 `koko_pdo()`、`koko_mysqli()` 和 `koko_require_admin()`。
- 新增 `admin/lib/mailer.php`：统一创建 PHPMailer 实例，SMTP Host、Port、Username、Password、From、To 全部来自环境变量或 `.env`，业务脚本只保留邮件主题和正文差异。
- 已移除 `admin/*.php`、`admin/phpapi/*.php` 中发现的硬编码数据库密码、QQ 邮箱、SMTP 授权码、发件人/收件人配置；`admin/vendor/` 是 Composer 依赖目录，本次保留不删除。
- 以下 PHP 写操作/敏感接口现在需要 `X-Admin-Token`：`admin/api.php?action=check_and_update_c_status`、`admin/api.php?action=setManualSuccess`、`admin/phpapi/transfer_in.php`、`admin/phpapi/transfer_out.php`、`admin/mail_test.php`、`admin/email.php`、`admin/cron_balance_monitor.php`；`admin/cron_auto_sync.php`、`admin/email.php`、`admin/cron_balance_monitor.php` 如果通过 Web 访问也需要 token，CLI cron 执行不需要 Header。
